...
enum VehicleSnapshotMapper {
    ...
    private static func mapCharge(_ pb: CarServer_ChargeState) -> ChargeState {
        // Prefer usable SoC when present; only map fields whose oneof is set
        // (SPM previously coerced unset → 0 and froze UI on first non-zero).
        let batteryLevel: Int? = {
            if pb.optionalUsableBatteryLevel != nil {
                return Int(pb.usableBatteryLevel)
            }
            if pb.optionalBatteryLevel != nil {
                return Int(pb.batteryLevel)
            }
            return nil
        }()
        let batteryRangeMiles: Double? = pb.optionalBatteryRange != nil
            ? Double(pb.batteryRange) : nil
        let estBatteryRangeMiles: Double? = pb.optionalEstBatteryRange != nil
            ? Double(pb.estBatteryRange) : nil
        let homeLat: Double? = pb.optionalHomeLocation != nil
            ? Double(pb.homeLocation.latitude) : nil
        let homeLng: Double? = pb.optionalHomeLocation != nil
            ? Double(pb.homeLocation.longitude) : nil
        let workLat: Double? = pb.optionalWorkLocation != nil
            ? Double(pb.workLocation.latitude) : nil
        let workLng: Double? = pb.optionalWorkLocation != nil
            ? Double(pb.workLocation.longitude) : nil

        return ChargeState(
            batteryLevel: batteryLevel,
            batteryRangeMiles: batteryRangeMiles,
            estBatteryRangeMiles: estBatteryRangeMiles,
            ...
            chargeRateMph: pb.optionalChargeRateMph != nil ? Double(pb.chargeRateMph) : nil,
    ...
    private static func mapDrive(_ pb: CarServer_DriveState) -> DriveState {
        ...
        let routeLat: Double? = pb.hasActiveRouteCoordinates
            ? Double(pb.activeRouteCoordinates.latitude) : nil
        let routeLng: Double? = pb.hasActiveRouteCoordinates
            ? Double(pb.activeRouteCoordinates.longitude) : nil
